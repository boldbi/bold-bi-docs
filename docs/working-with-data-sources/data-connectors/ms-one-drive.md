...
The Bold BI dashboard designer supports connecting to Microsoft OneDrive web services through the REST API. 
...
Use the following steps to authenticate with the Microsoft OneDrive web service:

1. Click on the data source, and you will be prompted with a login window. Enter the credentials of your Microsoft OneDrive account to authorize.
...
5. To connect via an already connected account, refer to [Connected Accounts](/working-with-data-sources/data-connectors/ms-one-drive/#connected-accounts-for-oauth-data-sources).
...
After successful authentication, the File Explorer panel opens. Follow the below steps to create Microsoft OneDrive data source.
...
4. Choose a time interval for **Refresh Settings** using the combo box to trigger the request periodically and keep the data in sync with our dashboard. 
...
For more information on the API endpoints available for this data source, refer to the official [Microsoft OneDrive API documentation](https://docs.microsoft.com/en-us/graph/auth-register-app-v2) 
...
> **NOTE:**  Multiple sheets in the `xlsx` file will be listed in a dropdown menu on the right side, as shown in the image. The maximum file size for the files to be previewed is `75 megabytes`.
