...
You can send a webhook notification for an export schedule or [data alerts](/working-with-dashboards/data-alerts/) by selecting the created webhook in `Other Destinations` section.
...
5. When running the schedule, Bold BI will call the webhook endpoint you selected with the selected or all payload data. Click [here](/manage-webhooks/webhook-payload/) to learn more about what is the payload data sent to a webhook call.
...
## Data Alerts
Data alert provides the users to track certain conditions and helps responding to a specified recipients via mail when a certain predefined threshold is met. Please find more details on the [Data alerts](/working-with-dashboards/data-alerts/).