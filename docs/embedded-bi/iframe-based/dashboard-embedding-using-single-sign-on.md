...
* The Dashboard Parameter filter must be started with a double ampersand `&&` in the embed URL. Refer to this [link](/embedded-bi/working-with-dashboards/configuring-dashboard-parameters/) for more details.    

* The URL filter Parameter must be started with a single ampersand `&` in the embed URL. Refer to this [link](/embedded-bi/visualize-data/urlparameters/) for more details.
