...
## Exporting dashboard to Excel
   ...
   ![Export to excel](/static/assets/working-with-dashboards/preview-dashboards/images/exporttoexcel.png#max-width=55%)
   
   By clicking `Export to Excel`, the pop-up will be shown as follows:
   
   ![Excel popup](/static/assets/working-with-dashboards/preview-dashboards/images/Excelpopup.png#max-width=55%)
   
   Set the `File Name` field with a preferred value to replace the default one.
   
   Choose the `Format` as either Excel Workbook (*.xlsx) or Excel 97-2003 Workbook (*.xls).
   
   ![Excel popup](/static/assets/working-with-dashboards/preview-dashboards/images/ExcelFormat.png#max-width=55%)

   When the `include filter information` is switched on, the exported Excel file of the dashboard will contain the applied filter information.

   ![Excel export](/static/assets/working-with-dashboards/preview-dashboards/images/excel-toggle-button.png#max-width=70%)
   
   Click `Export` to display the data in the dashboard in the chosen Excel format.
   
   ![Excel export](/static/assets/working-with-dashboards/preview-dashboards/images/Excelexport.png#max-width=55%)
   
   Click `Reset` to restore the default values in the pop-up.

   ![Excel reset](/static/assets/working-with-dashboards/preview-dashboards/images/Excelreset.png#max-width=55%)

   ### Export the user selected widgets as Excel

   By choosing the `Widgets` option, the dropdown will be displayed on the right side of the popup. By clicking the dropdown, all widgets in the dashboard will be displayed (excluding Filter Widgets). Users can choose the widgets required and click on Export to export selected widgets into Excel format.

   ![Excel Specific Widgets](/static/assets/working-with-dashboards/preview-dashboards/images/specificwidgetsexcelexport.png#max-width=55%)
   
   By clicking `Export`, the data in the dashboard will be downloaded with each widget on an separate sheet. 
   
   ![Excel Export Option](/static/assets/working-with-dashboards/preview-dashboards/images/excelexportoption.png#max-width=55%)
   
   By clicking `Reset`, the default values will be restored in the pop-up.

   ![Excel Reset](/static/assets/working-with-dashboards/preview-dashboards/images/excelexportreset.png#max-width=55%)
...
## Exporting dashboard to CSV
   ...
   By enabling `Disable Double Quotes`, the exported file will exclude double quotes from the column headers and values.

   ![CSV Popup](/static/assets/working-with-dashboards/preview-dashboards/images/CsvExportDisableDoubleQuotes.png)

   When the `include filter information` is switched on, the exported CSV file of the dashboard will contain the applied filter information.

   ![CSV Popup](/static/assets/working-with-dashboards/preview-dashboards/images/csv-toggle-button.png)
